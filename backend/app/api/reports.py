...
def portfolio_csv(db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> StreamingResponse:
    ...
    for project in projects:
        writer.writerow([safe_csv_value(value) for value in [report_id, project.id, project.title, project.state, project.district, project.risk_score, project.risk_level.value, project.sanctioned_lakh, project.spent_lakh, project.physical_progress, user.email]])
